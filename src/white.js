var White = {
  ...
  run: function(selector) {
    ...
    this.factor = (259 * (this.contrast + 255)) / (255 * (259 - this.contrast)); // used for contrast filter
    this.initImages();
  	for (var img in this.images) {
  		this.applyFilter(img);
  	}
  },
  initImages: function() {
  ...
  applyFilter: function(index) {
    ...
    img_info.vimg.onerror = function() {
      if (img_info.proxy < that.proxy.length-1) { // Not all tried
        img_info.proxy += 1;
        that.applyFilter(index);
      } else {  // Tried all proxies
